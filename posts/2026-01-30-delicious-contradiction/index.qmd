...
fig_static <- plot_ly(df_cumul, x = ~year) %>%
  ...
  add_trace(
    ...
    line = list(color = INFERNO$lineB, width = 3, dash = 'dash'),
    hovertemplate = 'Year: %{x}<br>%{y:.1f}%<extra></extra>'
  ) %>%
  layout(
    title = list(
      text = "Annual Returns and Cumulative Average",
      font = list(size = 14),
      xref = "paper",
      x = 0.5,
      xanchor = "center"
    ...
    plot_bgcolor = "white",
    paper_bgcolor = "white",
    margin = list(l = 50, r = 20, t = 50, b = 60, pad = 4),
...
It shows a set of data on US stock market returns from 1871 to 2025. This plot has two traces, purple solid trace and orange dashed trace. Purple solid trace represents yearly nominal returns, expressed in percentages, %. The line helps to guide your sight from datapoint to datapoint. If you hover your finger over purple trace, you can see returns for a particular year. Eg. it was -24.5% in 1975, 33.5% in 1976 and 7.5% in 1977. Overall, when you look at the purple trace you see many ups and downs. It is hard to answer the question “what typically the US stock market returns?” On the other hand, orange dashed line shows what happens when you cumulatively average yearly returns. The trace starts bumpy but smoothens out the further from the start you are. You see that from ~ year 2000 the returns average to 6% yearly. With this trace you can immediately answer the question: what did the stock market return on average from 1871 to 1950? (and now hover over orange dashed line for 1950). A bonus: if you made it that far in the post, you can isolate the orange trace by clicking on it in the plot legend. 
...
fig_interactive <- plot_ly() %>%
  layout(
    ...
    xaxis = list(title = "Year", automargin = TRUE),
    yaxis = list(title = "Return, %", automargin = TRUE),
    plot_bgcolor = "white",
    paper_bgcolor = "white",
    margin = list(l = 50, r = 20, t = 50, b = 60, pad = 4),
    autosize = TRUE
